...
def fn_process_menu_choice(choice_made):
    ...
    if choice_made == "1":
        ...
        print(f"The Result of adding {x} and {y} is {fn_add_two_numbers (x,y)}")
    elif choice_made =="2":
        x = input("Input the first number: ")
        y = input("Input the second number:")
        print(
        ...
        print(f"The number you entered was {starting_base}, the next number is {fn_nextNumber(int(starting_base))}")
    elif choice_made == "4":
        mins = input("Enter the number of minutes to be converted to seconds: ")
        print (f"There are {fn_min_to_seconds(int(mins))} seconds in {mins} minutes")
    elif choice_made =="5":
...
def fn_how_long (my_year, my_month, my_day):
    birth_date = datetime.date(my_year,my_month,my_day)
    now_date=datetime.now()
    print(f"You have been alive for a total of {abs(now_date - birth_date).days} days")
    print(f"You have been alive for a total of {abs(now_date - birth_date).years} years")
